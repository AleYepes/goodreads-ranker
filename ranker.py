...
def refine_ratings(target_df, rating_col, conn, interactive=False, title_col='title'):
    ...
    cursor = conn.execute("SELECT book_id, original_rating, elo_score, matches_played FROM elo_ratings")
...
def get_similar_friend_ratings(
    ...
    X_me = embedding_matrix[[book_id_to_idx[bid] for bid in my_train_ids]]
    ...
    knn_me.fit(X_me, y_me)

    friend_scores = []
    calibrated_friend_rows = []

    for list_id, friend_df in friends.groupby('list_id'):
        overlap = friend_df.merge(my_books, on='book_id', how='inner') \
                           .dropna(subset=['rating', 'my_refined']).copy()
        ...
        if my_only_ids and len(friend_df) >= 2:
            ...
            X_friend = embedding_matrix[[book_id_to_idx[bid] for bid in friend_df['book_id']]]
            ...
            knn_friend.fit(X_friend, y_friend)

            X_my_only = embedding_matrix[[book_id_to_idx[bid] for bid in my_only_ids]]
            predicted_friend = knn_friend.predict(X_my_only)
        ...
        if friend_only_ids:
            X_friend_only = embedding_matrix[[book_id_to_idx[bid] for bid in friend_only_ids]]
            predicted_me = knn_me.predict(X_friend_only)
...
def build_adjacency_matrix(books_df, num_nodes):
    ...
    adj_matrix = torch.sparse_coo_tensor(edge_index_norm, edge_weight_norm, (num_nodes, num_nodes))
    return adj_matrix

def prep_optimization(books_df, precomputed_embeddings, training_col, mrl_dimensions, max_propagations, budget=300):
...
def run_ranking(interactive=False, optimize=False, db_path=None):
    ...
    gr_export['my_rating'] = pd.to_numeric(gr_export['my_rating'], errors='coerce').replace(0, np.nan)
    ...
    if len(missing_ids) > 0:
        print(f"Warning: {len(missing_ids)} books are missing embeddings in database. Run embedder first to fill.")
    ...
    if optimize:
        ...
        friend_params = prep_optimization(books_df, precomputed_embeddings, 'training_ratings', mrl_dimensions, max_propagations, budget=200)
        solo_params = prep_optimization(books_df, precomputed_embeddings, 'my_refined', mrl_dimensions, max_propagations, budget=200)
    else:
    ...
    def weighted_rating(x, m=m, C=C):
        ...
        R = float(x['avg_rating']) if pd.notna(x['avg_rating']) else C
        if v == 0: 
            return C
    ...
    db.upsert_rows(
        ...
        "predictions",
        predictions_data,
        ["book_id", "solo_pred_rating", "friend_pred_rating", "count_adjusted_rating", "pred_rating", "final_rating", "updated_at"]
    )
    
    print(f"Successfully computed and saved predictions for {len(predictions_data)} books.")
    conn.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run modeling and prediction ranking pipeline.")
    parser.add_argument("--interactive", action="store_true", help="Run interactive ELO rating refinement")
    parser.add_argument("--optimize", action="store_true", help="Tune model hyperparameters using Nevergrad")
